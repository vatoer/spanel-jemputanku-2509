...
export function ArmadaTanstackTable({ data = defaultData }: ArmadaTanstackTableProps) {
  ...
  const handleRowClick = (armada: Armada) => {
    const platNomor = armada.platNomor.replace(/\s/g, "");
    router.push(`/armada/${platNomor}`);
  };

  const getTypeBadge = (tipe: string) => {
    const typeClasses = {
      "Bus Besar": "bg-blue-100 text-blue-700",
      "Bus Sedang": "bg-purple-100 text-purple-700",
      "Bus Kecil": "bg-green-100 text-green-700"
    };
    
    return (
      <span className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-md ${typeClasses[tipe as keyof typeof typeClasses] || "bg-gray-100 text-gray-700"}`}>
        {tipe}
      </span>
    );
  };

  const handleViewDetail = (e: React.MouseEvent, armada: Armada) => {
    e.stopPropagation();
    const platNomor = armada.platNomor.replace(/\s/g, "");
    router.push(`/armada/${platNomor}`);
  };

  const handleEdit = (e: React.MouseEvent, armada: Armada) => {
    e.stopPropagation();
    const platNomor = armada.platNomor.replace(/\s/g, "");
    router.push(`/armada/${platNomor}/edit`);
  };

  const handleDelete = async (e: React.MouseEvent, armada: Armada) => {
    e.stopPropagation();
    if (confirm(`Apakah Anda yakin ingin menghapus armada ${armada.platNomor}?`)) {
      try {
        console.log("Deleting armada:", armada.platNomor);
  ...
  const columns: ColumnDef<Armada>[] = [
      ...
      cell: ({ row }) => (
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2">
            {getTypeBadge(row.original.tipe)}
            <span className="text-xs text-gray-500">
              {row.original.merk} • {row.original.tahun}
            </span>
          </div>
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="space-y-1">
          <div className="font-medium text-gray-900">
            {row.original.kapasitas} penumpang
          </div>
          <div className="text-xs text-gray-500">
            {row.original.warna} • {row.original.fitur?.length || 0} fitur
          </div>
        </div>
      ),
    },
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="space-y-1">
                  ...
                  <span className="font-medium">{row.original.merk}</span>
                </div>
                <div className="space-y-1">
                  <span className="text-gray-500 block">Tahun</span>
                  <span className="font-medium">{row.original.tahun}</span>
                </div>
                <div className="space-y-1">
                  <span className="text-gray-500 block">Kapasitas</span>
                  <span className="font-medium">{row.original.kapasitas} penumpang</span>
                </div>
                <div className="space-y-1">
                  <span className="text-gray-500 block">Warna</span>
                  <span className="font-medium">{row.original.warna}</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                <div className="text-xs text-gray-500">
                  {row.original.fitur?.length || 0} fitur tersedia
                </div>
                <div className="flex items-center gap-1">
                  <Button
                    variant="ghost"
