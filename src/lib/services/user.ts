...
import type {
 ...
 export async function getUsersByTenant(
   ...
   */
  export async function removeUserFromTenant(userId: string, tenantId: string): Promise<RemoveUserFromTenantResult> {
    return await prisma.$transaction(async (tx) => {
      // Remove from tenant
   ...
   */
  export async function getUserPermissions(userId: string, tenantId: string): Promise<GetUserPermissionsResult> {
   ...
   */
  export async function getUserActivity(userId: string, limit: number = 20): Promise<GetUserActivityResult> {
